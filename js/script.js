const WHATSAPP_NUMBER = '5517997247866';
...
const galleryTriggers = document.querySelectorAll('[data-gallery-images]');
...
const updateCartCount = () => {
  ...
  document.querySelectorAll('[data-cart-word]').forEach((node) => {
    node.textContent = total === 1 ? 'item' : 'itens';
  });
};
...
const buildWhatsAppUrl = () => {
  ...
  const message = cart.length
    ? `Oi, Rayanni! Tenho interesse nestes itens:\n\n${lines.join('\n')}\n\nPode me passar valores e frete?`
    : 'Oi, Rayanni! Vim pelo site e gostaria de saber mais sobre suas artes.';
  ...
  if (!cart.length) {
    cartItemsNode.innerHTML = `
      <div class="empty-cart">
        ...
        <p>Volte para a página de artes ou de necessaires e adicione os itens que chamarem atenção.</p>
...
const createImageLightbox = () => {
  ...
  lightbox.innerHTML = `
    <div class="image-lightbox__panel" role="dialog" aria-modal="true" aria-label="Galeria de imagens">
      <button class="image-lightbox__close" type="button" aria-label="Fechar imagem">x</button>
      <button class="image-lightbox__control image-lightbox__control--prev" type="button" aria-label="Imagem anterior">&lt;</button>
      <img src="" alt="" data-lightbox-image />
      <button class="image-lightbox__control image-lightbox__control--next" type="button" aria-label="Próxima imagem">&gt;</button>
  ...
  const prevButton = lightbox.querySelector('.image-lightbox__control--prev');
  const nextButton = lightbox.querySelector('.image-lightbox__control--next');
  let images = [];
  let current = 0;
  ...
  const renderLightbox = () => {
    ...
    count.textContent = images.length > 1 ? `${current + 1} de ${images.length}` : '';
  ...
  const openLightbox = (trigger) => {
    ...
    current = 0;
    title.textContent = trigger.dataset.galleryTitle ?? 'Imagem do produto';
    renderLightbox();
